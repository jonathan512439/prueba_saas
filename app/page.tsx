...
function CatalogView({ rubro, productId, favorites, cart, toggleFavorite, addToCart, notify }: { rubro: Rubro; productId?: string; favorites: string[]; cart: CartLine[]; toggleFavorite: (r: string, p: string) => void; addToCart: (line: CartLine) => void; notify: (message: string) => void }) {
  ...
  return <main className="catalog-page" style={{ '--brand': rubro.color, '--brand-accent': rubro.accent } as React.CSSProperties}>
    ...
    <div className="catalog-review-wrap"><MapsReviewButton brand={rubro.brand} notify={notify} /></div>
    <section className="catalog-content" ref={productRef}><div className="catalog-categories"><button className="filter-button" onClick={() => notify('Filtros preparados')}><SlidersHorizontal size={17} /> Filtros</button>{catalog.categories.map((item) => <button key={item} className={category === item ? 'active' : ''} onClick={() => setCategory(item)}>{item}</button>)}</div><div className="benefit-row"><span><PackageCheck /> Stock actualizado</span><span><MessageCircle /> Respuesta rápida</span><span><BadgeCheck /> Compra segura</span><span><MapPin /> Cobertura local</span></div><div className="products-heading"><div><span className="eyebrow">{category === 'Todo' ? 'Selección para ti' : category}</span><h2>{query ? `Resultados para “${query}”` : 'Productos y servicios destacados'}</h2></div><button onClick={() => { setCategory('Todo'); setQuery(''); }}>Ver todo <ArrowRight size={16} /></button></div>{products.length ? <div className="product-grid">{products.map((product) => <ProductCard key={product.id} product={product} rubro={rubro} favorite={favorites.includes(`${rubro.id}:${product.id}`)} onFavorite={toggleFavorite} />)}</div> : <EmptyState action={() => setQuery('')} />}<section className="promo-band"><div><span>Atención personalizada</span><h2>¿No encuentras exactamente lo que necesitas?</h2><p>Cuéntanos qué buscas y te ayudaremos a elegir la mejor opción.</p></div><Button onClick={() => whatsapp()}><MessageCircle /> Hablar por WhatsApp</Button></section></section>
